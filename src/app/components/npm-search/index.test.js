...
describe("NpmSearchComponent", () => {
  ...
  it("displays results when API fetch succeeds", async () => {
    ...
    await waitFor(() => {
      ...
      expect(getByTestId("package", { container: result })).toHaveTextContent(
        mockData[0].package.name,
      );
      expect(getByTestId("version", { container: result })).toHaveTextContent(
        mockData[0].package.version,
      );
      expect(
        getByTestId("description", { container: result }),
      ).toHaveTextContent(mockData[0].package.description);
    });
  });
});
